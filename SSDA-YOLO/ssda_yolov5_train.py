...
def train(hyp, opt, device):
    ...
    save_checkpoint132 = wdir / 'save_checkpoint132.pt'
    save_checkpoint66 = wdir / 'save_checkpoint66.pt'
    save_checkpoint0 = wdir / 'save_checkpoint0.pt'
    ...
    for epoch in range(start_epoch, epochs):  # start epoch ------------------------------------------------------------
        ...
        if RANK in [-1, 0]:
            ...
            if not notest or final_epoch:  # Calculate mAP
                ...
                # Calculate mAP@50
                if epoch %20 == 0:
                    val_mAP50_current = float(results[2])
                    val_mAP50.append(val_mAP50_current)
                    epochs_save.append(epoch)
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ...
                if epoch==0:
                    torch.save(ckpt_student, save_checkpoint0)
                if epoch==66:
                    torch.save(ckpt_student, save_checkpoint66)
                if epoch==132:
                    torch.save(ckpt_student, save_checkpoint132)
    ...
    plt.figure()  # 创建一个新的图像
    plt.plot(epochs_save, val_mAP50, marker='o', markerfacecolor='red')
    plt.xlabel('Epochs')
    plt.ylabel('val_mAP50')
    plt.title('Validation mAP@0.5')
    plt.grid(True)
    plt.savefig('val_map50.png')
